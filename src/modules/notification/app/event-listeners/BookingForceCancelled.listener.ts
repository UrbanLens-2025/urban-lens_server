...
@Injectable()
export class BookingForceCancelledListener extends CoreService {
  constructor(
  ...
  @OnEvent(BOOKING_FORCE_CANCELLED_EVENT)
  handleEvent(event: BookingForceCancelledEvent) {
    return this.ensureTransaction(null, async (em) => {
      ...
      if (account) {
        await this.emailNotificationService.sendEmail({
          to: account.email,
          template: EmailTemplates.BOOKING_FORCE_CANCELLED,
          context: {
        ...
        await this.firebaseNotificationService.sendNotificationTo({
          toUserId: account.id,
          type: NotificationTypes.BOOKING_FORCE_CANCELLED,
          context: {
            locationName: booking.location.name,
          },
        });
